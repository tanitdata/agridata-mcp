...
from tanitdata.utils.formatting import PORTAL_BASE, format_dataset_list, format_source_footer
...
async def search_datasets(
    ...
    body = header + format_dataset_list(datasets)
    body += "\n" + format_source_footer([{
        "dataset_title": "Portal Search",
        "organization_title": "catalog.agridata.tn",
        "resource_id": "package_search",
        "portal_url": "https://catalog.agridata.tn",
    }])
    return body
...
async def get_dataset_details(client: CKANClient, dataset_id: str) -> str:
    ...
    lines.append("")
    lines.append(format_source_footer([{
        "dataset_title": title,
...
async def list_organizations(client: CKANClient, query: str = "") -> str:
    ...
    lines.append("")
    lines.append(format_source_footer([{
        "dataset_title": "Organization List",
        "organization_title": "catalog.agridata.tn",
        "resource_id": "package_search",
        "portal_url": "https://catalog.agridata.tn",
    }]))
