...
from tanitdata.utils.formatting import SKIP_COLS, format_source_footer
...
def _records_table(records: list[dict], max_rows: int = 50) -> list[str]:
    ...
    if not records:
        return ["*No records.*"]
    cols = [k for k in records[0] if k not in SKIP_COLS]
    lines = [
        "| " + " | ".join(cols) + " |",
        "| " + " | ".join("---" for _ in cols) + " |",
    ]
...
def _build_rainfall_sql(
    ...
    select_cols = [f'"{f}"' for f in fields if f not in SKIP_COLS]
