...
An AI-powered [MCP](https://modelcontextprotocol.io) server that turns Tunisia's agricultural open data portal ([agridata.tn](https://catalog.agridata.tn)) into a conversational data source. [MCP](https://modelcontextprotocol.io) (Model Context Protocol) lets AI assistants call external tools — connect your AI client and start asking questions in natural language.

The portal covers climate, crops, fisheries, water resources, livestock, prices, trade, and more. tanitdata handles querying structured DataStore tables and bibliographic catalogs so researchers can access both through conversation.
...
| **query_climate_stations** | Query Tunisia's weather monitoring network — temperature, rainfall, humidity, wind, and solar sensors. Supports time series, aggregation, multi-station comparison. *"Compare temperature trends between Bizerte and Nabeul this winter"* |

### Knowledge

| Tool | What it does |
|---|---|
| **search_bibliography** | Search ONAGRI's bibliographic catalogs (25,000+ records) by keyword, year, language, or theme. Returns ranked results with PDF links. *"Find French-language studies on water management published after 2015"* |
...
> Uses `search_bibliography` to search 25,000+ bibliographic records, returning ranked results with PDF download links.
...
> Uses `query_climate_stations` with no arguments to return a full inventory — all sensor stations grouped by governorate with live sensor lists.
...
- **Format mismatches** — some resources have CSV/XLSX format mislabeling; the server handles this transparently
- **Arabic field encoding** — some resources have mojibake in Arabic field names; Bizerte price datasets are auto-decoded
...
Data sourced from [catalog.agridata.tn](https://catalog.agridata.tn) under the Licence Nationale de Données Publiques Ouvertes.
